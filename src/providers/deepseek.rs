//! Dedicated provider for DeepSeek.
//!
//! Based on the Volcengine provider, with these differences:
//! - Uses `https://api.deepseek.com` as the base URL.
//! - Does NOT send `ThinkingConfig` on any request.
//! - Handles SSE parsing and tool-call delta accumulation identically.
...
impl DeepseekProvider {
    ...
    fn build_client() -> reqwest::Client {
        crate::config::build_runtime_proxy_client_with_timeouts("deepseek", 300, 10)
    }
}
...
impl Provider for DeepseekProvider {
    ...
    async fn chat_with_system(
        ...
        if !resp.status().is_success() {
            return Err(super::api_error("deepseek", resp).await);
        }
